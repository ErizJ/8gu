...
MCP 协议在 2025 年的更新中引入了 **Streamable HTTP** 作为**推荐的传输方式**，逐步替代 SSE。
...
> MCP 有四种 Transport：STDIO 用于本地工具（简单、进程隔离），HTTP 用于简单远程调用，SSE 是过渡方案（已被淘汰），Streamable HTTP 是远程场景的推荐首选——单端点双向流式通信，代理友好，全平台支持。
...
> - **过渡方案**：HTTP Transport（简单请求-响应，不支持推送）和 SSE Transport（已被 Streamable HTTP 取代）仅用于兼容旧系统。
>
...
| `notifications/cancelled` | 请求取消通知 |
| `notifications/resources/updated` | 资源内容变更通知 |
...
// Tool 处理函数
func handleGetWeather(ctx context.Context, args map[string]interface{}) (*mcp.CallToolResult, error) {
...
func (s *MySQLMCPServer) handleQuery(ctx context.Context, args map[string]interface{}) (*mcp.CallToolResult, error) {
...
func (s *MySQLMCPServer) handleListTables(ctx context.Context, args map[string]interface{}) (*mcp.CallToolResult, error) {
...
func (s *MySQLMCPServer) handleDescribeTable(ctx context.Context, args map[string]interface{}) (*mcp.CallToolResult, error) {
...
> 开发 MCP Server 的核心步骤：
>
> 1. **选择 SDK**：Python 用 FastMCP（高层封装），Go 用 go-sdk（底层 JSON-RPC）
...
> MCP 通过多层安全机制保证安全：
>
> 1. **用户授权层**：首次调用工具时弹窗确认，用户可选择允许/拒绝/始终允许
> 2. **Tool 级权限**：每个 Tool 可独立设置权限（只读/读写/禁止）
> 3. **Resource 级权限**：URI 模式匹配的访问控制（白名单/黑名单）
> 4. **进程隔离**：STDIO 模式下 Server 作为子进程运行，操作系统级隔离
> 5. **传输层安全**：HTTP 传输使用 TLS 加密
...
func (p *AuditProxy) CallTool(ctx context.Context, name string, args map[string]interface{}) (*ToolResult, error) {
    // 1. 调用前检查
...
>
> **Streamable HTTP Transport**：这是 2025 年推出的推荐远程方案。单一 POST 端点，在同一个 HTTP 连接上实现双向流式通信。相比被淘汰的 SSE（双通道复杂、代理不友好），Streamable HTTP 代理友好、支持断点续传、全平台兼容。适合**生产环境和远程服务**。
>
...
>
> **第三步，注册 Tools**：这是核心工作。每个 Tool 需要 name（snake_case，动词开头）、description（越详细越好，这是 LLM 选择工具的唯一依据）、inputSchema（JSON Schema 定义参数类型、枚举、默认值、必填项）、handler（实际的业务逻辑函数）。
>
...
> "MCP 安全是一个纵深防御体系，至少六层：
>
> **第一层，用户授权**：首次调用 Tool 弹窗确认，用户可选择允许/拒绝/始终允许。这是最直接的人机交互防线。
>
> **第二层，Tool 级权限**：每个 Tool 独立配置——只读、可写需确认、禁止。比如 `read_file` 允许，`delete_file` 需确认，`execute_shell` 禁止。
>
> **第三层，Resource 级权限**：URI 级别的白名单/黑名单。`file:///workspace/**` 允许，`file:///etc/**`（系统配置）禁止。
>
> **第四层，进程/容器隔离**：STDIO 模式天然进程隔离；生产环境用 Docker 容器做额外隔离——只读文件系统、禁止提权、独立网络栈。
>
> **第五层，传输安全**：远程通信强制 TLS 加密。
>
> **第六层，Server 端校验**：永远不信任 Client 传来的参数——SQL 注入防护、路径穿越检查、输入大小限制。
>
...
> 以下 20 道题是 MCP 面试最高频的问题。每题提供了**可直接背诵的标准回答**。建议先理解前面各章的详细内容，再用这里的精简版进行最后冲刺。
...
**标准回答：**
> Tool 是 MCP 中最核心的概念，代表 **LLM 可以调用的一个功能单元**。每个 Tool 由三要素定义：
...
**关键词**：功能单元、name+description+inputSchema、tools/list 发现、tools/call 调用
...
> - **原理**：Host 启动 Server 作为子进程，通过 stdin 发送 JSON-RPC 请求，通过 stdout 接收 JSON-RPC 响应，stderr 用于日志。消息格式为 `Content-Length` 头 + JSON body。
...
**标准回答：**
> Streamable HTTP 是 MCP 2025 年推出的**推荐远程传输方案**，替代了 SSE：
...
**关键词**：单端点 POST、流式响应、替代 SSE、代理友好、断点续传
...
**关键词**：选 SDK → 创建 → 注册 Tool/Resource/Prompt → 启动、FastMCP/go-sdk
...
**关键词**：Gateway → Router → Proxy → Server 集群 → 基础设施、五层架构、服务发现
...
| `2025-03-26` | 2025-03 | 引入 Streamable HTTP，弃用 SSE |
| `2025-06-18` | 2025-06 | 增强 Resource 模板，任务管理 |
