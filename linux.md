...
```bash
ip addr                         # 查看所有网络接口的 IP
ip addr show eth0               # 查看特定网卡
ip route                        # 查看路由表
ip link show                    # 查看链路状态
ifconfig                        # 旧命令（需要 net-tools）
```
...
> **面试答法**：Linux 有五种 I/O 模型。生产环境统一用 I/O 多路复用（epoll）。Go 的 netpoller 底层用 epoll 实现，但对开发者暴露同步阻塞 API（`conn.Read()`），runtime 将阻塞的 goroutine 挂起，fd 就绪后再唤醒——"同步写法，异步执行"。这也是 Go 网络编程的核心优势。
...
debug.SetMemoryLimit(800 * 1024 * 1024)  // 800MB 软限制
...
# → Go 中 net.Listen 的 backlog 参数不能超过此值
# Go 中默认：net.Listen("tcp", ":8080") 内部调用 listen(fd, 128)
# 128 太小！应该设为：
# listener, _ := net.Listen("tcp", ":8080")
# 或使用 net.ListenConfig 调整 Backlog
